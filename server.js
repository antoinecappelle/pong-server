...
io.on('connection', (socket) => {
    ...
    socket.on('move', (y) => { socket.to(socket.roomId).emit('opponentMove', y); });
    socket.on('ballSync', (data) => { socket.to(socket.roomId).emit('ballUpdate', data); });

    socket.on('disconnect', () => {
        if (rooms[socket.roomId]) {
            socket.to(socket.roomId).emit('opponentLeft', "L'adversaire a quitté.");
